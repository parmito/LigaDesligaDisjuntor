...
static esp_err_t Read_get_handler(httpd_req_t *req)
{
    ...
    strcat(cWebpage,"<title>RIO WEB SERBER</title>\n");
	...
	strcat(cWebpage,"input[type=text] {");
	  ...
	  strcat(cWebpage,"padding: 10px 10px;");
	  strcat(cWebpage,"margin: 10px auto;");
	  strcat(cWebpage,"box-sizing: border-box;");
	  strcat(cWebpage,"border-radius: 8px;");
    ...
    strcat(cWebpage,".button {display: block;width: 120px;background-color: #3498db;color: white;padding: 10px 10px;font-size: 25px;cursor: pointer;border-radius: 8px;text-align:center; margin: 10px auto }\n");
    ...
    strcat(cWebpage,"<h1>RIO Web Server</h1>\n");
	...
	strcat(cWebpage,"<form action=\"/record\"  method=\"get\">");
	strcat(cWebpage,"<button class=\"button button-off\" id=\"record_button\">Record Keyless</button>");
	strcat(cWebpage,"</form>");


	strcat(cWebpage,"<script>");
	strcat(cWebpage,"document.getElementById(\"record_button\").addEventListener(\"waiting\", myFunction);");
	strcat(cWebpage,"function myFunction() {");
	strcat(cWebpage,"alert(\"Wait! I need to buffer the next frame\");");
	strcat(cWebpage,"}");
	strcat(cWebpage,"</script>");
...
static esp_err_t Record_get_handler(httpd_req_t *req)
{
    char*  buf;
    size_t buf_len;
